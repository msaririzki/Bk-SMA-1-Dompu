...
@section('content')
    <section class="relative overflow-hidden bg-navy-950 text-white">
        ...
        <div class="container-page relative grid items-center gap-14 py-20 lg:grid-cols-[1.05fr_.95fr] lg:py-28">
            <div>
                ...
                <h1 class="mt-7 max-w-3xl text-4xl font-extrabold leading-[1.08] tracking-[-.045em] text-white sm:text-6xl">Pendampingan siswa yang lebih tertata dan manusiawi.</h1>
                ...
                <div class="mt-9 flex flex-wrap gap-3"><a class="btn btn-accent" href="{{ route('login') }}"><x-icon name="lock" />Masuk sebagai guru BK</a><a class="btn border border-white/15 bg-white/5 text-white hover:-translate-y-0.5 hover:bg-white/10" href="{{ route('student.login') }}"><x-icon name="user" />Akses siswa</a></div>
            ...
            <div class="relative mx-auto w-full max-w-xl lg:mx-0">
                <div class="glass-card relative overflow-hidden p-4 sm:p-5">
                    <div class="flex items-center justify-between border-b border-white/[.08] pb-4"><div class="flex items-center gap-3"><span class="grid size-9 place-items-center rounded-xl bg-teal-400 font-black text-navy-950">BK</span><div><p class="text-sm font-bold text-white">Pratinjau dashboard</p><p class="text-[11px] text-slate-400">Contoh ringkasan layanan</p></div></div><span class="flex items-center gap-2 rounded-full bg-emerald-400/10 px-2.5 py-1 text-[10px] font-bold text-emerald-300"><span class="size-1.5 rounded-full bg-emerald-300"></span>Aman</span></div>
                    <div class="mt-4 grid grid-cols-3 gap-3">
                        @foreach ([['Siswa', '1.264'], ['Kasus bulan ini', '18'], ['Perlu tindak lanjut', '7']] as [$label, $value])
                            <div class="rounded-2xl border border-white/[.07] bg-white/[.05] p-3"><p class="text-[10px] font-semibold text-slate-400">{{ $label }}</p><p class="mt-1.5 text-xl font-extrabold text-white">{{ $value }}</p></div>
                        @endforeach
                    </div>
                    <div class="mt-3 rounded-2xl border border-white/[.07] bg-white/[.05] p-4"><div class="flex items-center justify-between"><div><p class="text-xs font-bold text-white">Prioritas penanganan</p><p class="mt-1 text-[10px] text-slate-400">Ringkasan berdasarkan poin tahunan</p></div><x-icon name="chart" class="size-5 text-teal-300" /></div><div class="mt-5 space-y-4">@foreach ([['Ringan', 78, 'bg-teal-400'], ['Sedang', 48, 'bg-amber-400'], ['Berat', 24, 'bg-orange-400']] as [$label, $width, $color])<div><div class="mb-1.5 flex justify-between text-[10px] font-semibold text-slate-300"><span>{{ $label }}</span><span>{{ $width }}%</span></div><div class="h-1.5 rounded-full bg-white/[.07]"><div class="h-1.5 rounded-full {{ $color }}" style="width: {{ $width }}%"></div></div></div>@endforeach</div></div>
                </div>
    ...
    <section class="container-page pb-4">
        <div class="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-navy-900 to-navy-800 px-6 py-10 text-white shadow-2xl shadow-navy-900/15 sm:px-10 lg:flex lg:items-center lg:justify-between lg:px-12"><div class="pointer-events-none absolute right-0 top-0 size-64 rounded-full bg-teal-400/10 blur-3xl"></div><div class="relative"><p class="text-xs font-bold uppercase tracking-[.16em] text-teal-300">Portal pribadi siswa</p><h2 class="mt-3 max-w-2xl text-2xl font-extrabold text-white sm:text-3xl">Lihat riwayat dan progres pelanggaran secara aman.</h2><p class="mt-3 max-w-xl text-sm leading-6 text-slate-300">Gunakan NIS, NISN, atau ID sementara dan PIN yang diberikan sekolah.</p></div><a class="btn btn-accent relative mt-7 shrink-0 lg:mt-0" href="{{ route('student.login') }}">Buka portal siswa <x-icon name="arrow-right" /></a></div>
    </section>
@endsection
