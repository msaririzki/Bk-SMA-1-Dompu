...
<div class="card overflow-hidden"><div class="table-wrap"><table class="table"><thead><tr><th>Nomor/tanggal</th><th>Siswa</th><th>Pelanggaran</th><th>Poin</th><th>Status</th><th><span class="sr-only">Aksi</span></th></tr></thead><tbody>@forelse($cases as $case)<tr><td><strong>{{ $case->case_number }}</strong><p class="mt-1 text-xs text-slate-400">{{ $case->occurred_at->translatedFormat('d M Y H:i') }}</p></td><td><strong>{{ $case->student->name }}</strong><p class="mt-1 text-xs text-slate-400">{{ $case->student->currentEnrollment?->schoolClass?->name }}</p></td><td><p class="max-w-sm line-clamp-2 leading-6">{{ $case->items->pluck('instrument_name')->join(', ') }}</p></td><td><span class="badge badge-amber">{{ $case->items->sum('points') }} poin</span></td><td><span class="badge badge-slate">{{ $case->status->label() }}</span></td><td class="text-right"><div class="flex justify-end gap-2"><a class="btn btn-secondary whitespace-nowrap" href="{{ route('students.recap', $case->student) }}"><x-icon name="download" /> Rekap PDF</a><a class="btn btn-secondary" href="{{ route('cases.show', $case) }}">Buka <x-icon name="arrow-right" /></a></div></td></tr>@empty<tr><td colspan="6"><div class="empty-state"><x-icon name="clipboard" class="mx-auto mb-3 size-8" /><p>Belum ada kasus.</p></div></td></tr>@endforelse</tbody></table></div></div>
